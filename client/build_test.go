...
import (
	...
	gatewayapi "github.com/khulnasoft/devkit/frontend/gateway/pb"
	...
	utilsystem "github.com/khulnasoft/devkit/util/system"
...
func testClientSlowCacheRootfsRef(t *testing.T, sb integration.Sandbox) {
	...
	_, err = c.Build(ctx, SolveOpt{}, "devkit_test", b, nil)
	require.NoError(t, err)
	checkAllReleasable(t, c, sb, true)
}
...
func testClientGatewayContainerHostNetworking(t *testing.T, sb integration.Sandbox) {
	...
	if sb.Rootless() && sb.Value("netmode") == defaultNetwork {
		...
		// https://github.com/khulnasoft/devkit/blob/v0.9.0/docs/rootless.md#known-limitations
		t.SkipNow()
	}
...
func testClientGatewayEmptyImageExec(t *testing.T, sb integration.Sandbox) {
	...
	require.NoError(t, err)
	target := registry + "/devkit/testemptyimage:latest"
