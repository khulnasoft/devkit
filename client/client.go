...
import (
	...
	"github.com/khulnasoft/devkit/client/connhelper"
	...
	"github.com/khulnasoft/devkit/util/tracing/otlptracegrpc"
...
// New returns a new devkit client. Address can be empty for the system-default address.
func New(ctx context.Context, address string, opts ...ClientOpt) (*Client, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to dial %q . make sure devkitd is running", address)
	}
...
func (c *Client) Wait(ctx context.Context) error {
	for {
		...
		case codes.Unimplemented:
			// only devkit v0.11+ supports the info api, but an unimplemented
