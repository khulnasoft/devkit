...
import (
	...
	gateway "github.com/khulnasoft/devkit/frontend/gateway/client"
	gatewaypb "github.com/khulnasoft/devkit/frontend/gateway/pb"
	"github.com/khulnasoft/devkit/identity"
	"github.com/khulnasoft/devkit/session"
	"github.com/khulnasoft/devkit/session/filesync"
	...
	sourcepolicypb "github.com/khulnasoft/devkit/sourcepolicy/pb"
	"github.com/khulnasoft/devkit/util/attestation"
	"github.com/khulnasoft/devkit/util/contentutil"
	"github.com/khulnasoft/devkit/util/entitlements"
	"github.com/khulnasoft/devkit/util/testutil"
	containerdutil "github.com/khulnasoft/devkit/util/testutil/containerd"
	"github.com/khulnasoft/devkit/util/testutil/echoserver"
	"github.com/khulnasoft/devkit/util/testutil/helpers"
	"github.com/khulnasoft/devkit/util/testutil/httpserver"
	"github.com/khulnasoft/devkit/util/testutil/integration"
	"github.com/khulnasoft/devkit/util/testutil/workers"
...
func testIntegration(t *testing.T, funcs ...func(t *testing.T, sb integration.Sandbox)) {
	...
	mirroredImages["cpuguy83/devkit-foreign:latest"] = "docker.io/cpuguy83/devkit-foreign:latest"
...
func testExportedImageLabels(t *testing.T, sb integration.Sandbox) {
	...
	target := "docker.io/devkit/build/exporter:labels"
	...
	ctx = namespaces.WithNamespace(ctx, "devkit")
	...
	// devkit contentstore still has the layer because it is multi-ns
...
func testSSHMount(t *testing.T, sb integration.Sandbox) {
	...
	require.NoError(t, err)
	require.Equal(t, "/run/devkit/ssh_agent.0", string(dt))
...
func testFrontendImageNaming(t *testing.T, sb integration.Sandbox) {
	...
	checkImageName := map[string]func(out, imageName string, exporterResponse map[string]string){
		...
		ExporterImage: func(_, imageName string, exporterResponse map[string]string) {
			...
			ctx := namespaces.WithNamespace(sb.Context(), "devkit")
...
func testBuildExportScratch(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/build/exporter:withnocompressed"
	...
	targetMulti := registry + "/devkit/build/exporter-multi:withnocompressed"
	_, err = c.Build(sb.Context(), SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
...
func testMultipleExporters(t *testing.T, sb integration.Sandbox) {
	...
	target1, target2 := registry+"/devkit/build/exporter:image",
		registry+"/devkit/build/alternative:image"
...
func testOCIExporter(t *testing.T, sb integration.Sandbox) {
	...
	for _, exp := range []string{ExporterOCI, ExporterDocker} {
		...
		outW, err := os.Create(out)
		require.NoError(t, err)
		target := "example.com/devkit/testoci:latest"
		attrs := map[string]string{}
...
func testSourceDateEpochLayerTimestamps(t *testing.T, sb integration.Sandbox) {
	...
	destDir, err := os.MkdirTemp("", "devkit")
	require.NoError(t, err)
...
func testBuildExportWithForeignLayer(t *testing.T, sb integration.Sandbox) {
	...
	st := llb.Image("cpuguy83/devkit-foreign:latest")
	def, err := st.Marshal(sb.Context())
	require.NoError(t, err)
	...
	t.Run("propagate=1", func(t *testing.T) {
		...
		target := registry + "/devkit/build/exporter/foreign:latest"
		_, err = c.Solve(sb.Context(), def, SolveOpt{
			Exports: []ExportEntry{
				{
					Type: ExporterImage,
	...
	t.Run("propagate=0", func(t *testing.T) {
		...
		require.NoError(t, err)
		target := registry + "/devkit/build/exporter/noforeign:latest"
		_, err = c.Solve(sb.Context(), def, SolveOpt{
			Exports: []ExportEntry{
				{
					Type: ExporterImage,
...
func testBuildExportWithUncompressed(t *testing.T, sb integration.Sandbox) {
	...
	compressedTarget := registry + "/devkit/build/exporter:withcompressed"
	_, err = c.Solve(sb.Context(), def, SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
	...
	allCompressedTarget := registry + "/devkit/build/exporter:withallcompressed"
	_, err = c.Solve(context.TODO(), def, SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
...
func testPullZstdImage(t *testing.T, sb integration.Sandbox) {
	...
	for _, ociMediaTypes := range []bool{true, false} {
		...
		t.Run(t.Name()+fmt.Sprintf("/ociMediaTypes=%t", ociMediaTypes), func(t *testing.T) {
			...
			target := registry + "/devkit/build/exporter:zstd"
...
func testBuildPushAndValidate(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testpush:latest"
...
func testStargzLazyRegistryCacheImportExport(t *testing.T, sb integration.Sandbox) {
	...
	def, err = baseDef.
		...
		Marshal(sb.Context())
	require.NoError(t, err)
	target := registry + "/devkit/testlazyimage:" + identity.NewID()
	resp, err := c.Solve(sb.Context(), def, SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
...
func testLazyImagePush(t *testing.T, sb integration.Sandbox) {
	...
	targetNoTag := registry + "/devkit/testlazyimage:"
	...
	target3 := registry + "/devkit/testlazycrossrepo:latest"
	_, err = c.Solve(sb.Context(), def, SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
...
func testUncompressedRegistryCacheImportExport(t *testing.T, sb integration.Sandbox) {
	...
	require.NoError(t, err)
	target := registry + "/devkit/testexport:latest"
	im := CacheOptionsEntry{
		Type: "registry",
		Attrs: map[string]string{
			"ref": target,
...
func testBasicInlineCacheImportExport(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testexportinline:latest"
	...
	for _, ociMediaTypes := range []bool{true, false} {
		...
		for _, imageManifest := range []bool{true, false} {
			...
			t.Run(t.Name()+fmt.Sprintf("/ociMediaTypes=%t/imageManifest=%t", ociMediaTypes, imageManifest), func(t *testing.T) {
				...
				cacheTarget := registry + "/devkit/testregistryemptycache:latest"
...
func testMultipleRecordsWithSameLayersCacheImportExport(t *testing.T, sb integration.Sandbox) {
	...
	// More context: https://github.com/khulnasoft/devkit/pull/3815
	ensurePruneAll(t, c, sb)
}

// khulnasoft/devkit#3809
...
func testMergeOp(t *testing.T, sb integration.Sandbox) {
	...
	if workers.IsTestDockerdMoby(sb) {
		...
		imageTarget = registry + "/devkit/testmergeop:latest"
	}
...
func testMergeOpCache(t *testing.T, sb integration.Sandbox, mode string) {
	...
	busyboxTargetNoTag := registry + "/devkit/testlazyimage:"
	...
	target := registry + "/devkit/testmerge:latest"
	cacheTarget := registry + "/devkit/testmergecache:latest"
...
func testPullWithLayerLimit(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testlayers:latest"
...
func testValidateDigestOrigin(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testdigest:latest"
...
func testExportAnnotations(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testannotations:latest"
...
func testExportAnnotationsMediaTypes(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testannotationsmedia:1"
	_, err = c.Build(sb.Context(), SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
	...
	target2 := registry + "/devkit/testannotationsmedia:2"
	_, err = c.Build(sb.Context(), SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
...
func testExportAttestations(t *testing.T, sb integration.Sandbox) {
	...
	t.Run("image", func(t *testing.T) {
		targets := []string{
			registry + "/devkit/testattestationsfoo:latest",
			registry + "/devkit/testattestationsbar:latest",
		}
		_, err = c.Build(sb.Context(), SolveOpt{
			Exports: []ExportEntry{
				{
...
func testAttestationDefaultSubject(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testattestationsemptysubject:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
	...
	for i, att := range atts.Images {
		...
		name := fmt.Sprintf("pkg:docker/%s/devkit/testattestationsemptysubject@latest?platform=%s", url.QueryEscape(registry), url.QueryEscape(platforms.Format(ps[i])))
		subjects := []intoto.Subject{{
			Name: name,
			Digest: map[string]string{
				"sha256": bases[i].Desc.Digest.Encoded(),
...
func testAttestationBundle(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/testattestationsbundle:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
	...
	for i, att := range atts.Images {
		...
		name := fmt.Sprintf("pkg:docker/%s/devkit/testattestationsbundle@latest?platform=%s", url.QueryEscape(registry), url.QueryEscape(platforms.Format(ps[i])))
		subjects := []intoto.Subject{{
			Name: name,
			Digest: map[string]string{
				"sha256": bases[i].Desc.Digest.Encoded(),
...
`
	...
	scannerTarget := registry + "/devkit/testsbomscanner:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		Exports: []ExportEntry{
			{
				Type: ExporterImage,
	...
	target := registry + "/devkit/testsbom:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		FrontendAttrs: map[string]string{
			"attest:sbom": "",
		},
	...
	target = registry + "/devkit/testsbom2:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		FrontendAttrs: map[string]string{
			"attest:sbom": "",
		},
	...
	target = registry + "/devkit/testsbom3:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		FrontendAttrs: map[string]string{
...
`
	...
	target := registry + "/devkit/testsbomsingle:latest"
	_, err = c.Build(sb.Context(), SolveOpt{
		FrontendAttrs: map[string]string{
			"attest:sbom": "generator=" + scannerTarget,
		},
...
// https://github.com/khulnasoft/devkit/issues/3148
...
[worker.oci]
networkMode = "cni"
cniConfigPath = "/etc/devkit/dns-cni.conflist"
...
func testSourcePolicy(t *testing.T, sb integration.Sandbox) {
	...
	frontend := func(ctx context.Context, c gateway.Client) (*gateway.Result, error) {
		st := llb.Image("busybox:1.34.1-uclibc").File(
			llb.Copy(llb.HTTP("https://raw.githubusercontent.com/khulnasoft/devkit/v0.10.1/README.md"),
				"README.md", "README.md"))
		def, err := st.Marshal(sb.Context())
		if err != nil {
			return nil, err
	...
	t.Run("Frontend policies", func(t *testing.T) {
		t.Run("deny http", func(t *testing.T) {
			denied := "https://raw.githubusercontent.com/khulnasoft/devkit/v0.10.1/README.md"
			frontend := func(ctx context.Context, c gateway.Client) (*gateway.Result, error) {
				st := llb.Image("busybox:1.34.1-uclibc").File(
