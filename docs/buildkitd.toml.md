...
The TOML file used to configure the devkitd daemon settings has a short
list of global settings followed by a series of sections for specific areas
of daemon configuration.

The file path is `/etc/devkit/devkitd.toml` for rootful mode,
`~/.config/devkit/devkitd.toml` for rootless mode.

The following is a complete `devkitd.toml` configuration example, please
...
trace = true
# root is where all devkit state is stored.
root = "/var/lib/devkit"
...
[grpc]
  ...
  gid = 0
  [grpc.tls]
    cert = "/etc/devkit/tls.crt"
    key = "/etc/devkit/tls.key"
    ca = "/etc/devkit/tlsca.crt"

[otel]
  # OTEL collector trace socket path
  socketPath = "/run/devkit/otel-grpc.sock"
...
[worker.oci]
  ...
  # running rootless devkit inside a container.
  ...
  # devkit-runc or runc binary is used
  binary = ""
