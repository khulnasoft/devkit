...
`buildctl` is the command-line interface to `devkitd`.
...
GLOBAL OPTIONS:
   ...
   --addr value           devkitd address (default: "unix:///run/devkit/devkitd.sock")
   --log-format value     log formatter: json or text (default: "text")
   --tlsservername value  devkitd server name for certificate validation
...
`buildctl` connects to a running `devkitd` instance. The connection can is in a URL format of `<protocol>://<address>`.
...
* Unix-domain socket via `unix://path/to/socket`, e.g. `unix:///run/devkit/devkitd.sock` (which is the default)
...
`buildctl build` uses a devkit daemon `devkitd` to drive a build.
...
The frontend is declared by the flag `--frontend <frontend>`. The `<frontend>` must be one built into `devkitd`, or an OCI
...
* `dockerfile.v0`: uses the dockerfile-to-LLB frontend converter that is built into devkitd.
...
* `--local <name>=<dir>` - allow devkitd to access a local-to-buildctl directory `<dir>` under the unique name `<name>`.
* `--oci-layout <name>=<dir>` - allow devkitd to access OCI images in the local-to-buildctl directory `<dir>` under the unique name `<name>`.
...
lets `devkitd` access all of the files in `/var/lib/test1` (relative to wherever `buildctl` is running), referenced via the name `test1`.
...
lets `devkitd` access OCI images under `/var/lib/oci` (relative to wherever `buildctl` is running), referenced via the name `foo`.
...
Cache defines options for devkit to do one or both of:
...
During the build process, `devkitd` generates cache layers. These can be exported at the end of the build via:
...
During the build process, `devkitd` uses its local cache to optimize its build. In addition, you
