...
FROM gobuild-base AS devkit-base
WORKDIR /src
ENV GOFLAGS=-mod=vendor

# scan the version/revision info
FROM devkit-base AS devkit-version
# TODO: PKG should be inferred from go modules
RUN --mount=target=. \
  PKG=github.com/khulnasoft/devkit VERSION=$(git describe --match 'v[0-9]*' --dirty='.m' --always --tags) REVISION=$(git rev-parse HEAD)$(if ! git diff --no-ext-diff --quiet --exit-code; then echo .m; fi); \
  echo "-X ${PKG}/version.Version=${VERSION} -X ${PKG}/version.Revision=${REVISION} -X ${PKG}/version.Package=${PKG}" | tee /tmp/.ldflags; \
  echo -n "${VERSION}" | tee /tmp/.version;

# build buildctl binary
FROM devkit-base AS buildctl
ENV CGO_ENABLED=0
ARG TARGETPLATFORM
...
RUN --mount=target=. --mount=target=/root/.cache,type=cache \
  ...
  # https://github.com/khulnasoft/devkit/pull/4491. Retry a few times as a workaround.
  set +ex
  if [ "$(xx-info os)" = "linux" ]; then
    max_retries=5
    for attempt in $(seq "$max_retries"); do
      timeout 3 /usr/bin/devkitd --version
      exitcode=$?
      if ! xx-info is-cross; then
        ...
        echo "WARN: devkitd --version timed out ($attempt/$max_retries)"
        if [ "$attempt" -eq "$max_retries" ]; then
          exit $exitcode
        fi
      else
        echo "ERROR: devkitd --version failed with exit code $exitcode"
      fi
      sleep 1
    done
  fi
...
COPY --link --from=cni-plugins /opt/cni/bin/bridge /devkit-cni-bridge
COPY --link --from=cni-plugins /opt/cni/bin/loopback /devkit-cni-loopback
COPY --link --from=cni-plugins /opt/cni/bin/host-local /devkit-cni-host-local
COPY --link --from=cni-plugins /opt/cni/bin/firewall /devkit-cni-firewall
...
COPY --link --from=runc /usr/bin/runc /devkit-runc
# built from https://github.com/tonistiigi/binfmt/releases/tag/devkit%2Fv7.1.0-30
COPY --link --from=tonistiigi/binfmt:devkit-v7.1.0-30@sha256:45dd57b4ba2f24e2354f71f1e4e51f073cb7a28fd848ce6f5f2a7701142a6bf0 / /
COPY --link --from=cni-plugins-export-squashed / /
COPY --link --from=buildctl /usr/bin/buildctl /
COPY --link --from=devkitd /usr/bin/devkitd /
...
RUN --mount=from=binaries \
  --mount=source=/tmp/.version,target=/tmp/.version,from=devkit-version \
  mkdir -p /out && tar czvf "/out/devkit-$(cat /tmp/.version).$(echo $TARGETPLATFORM | sed 's/\//-/g').tar.gz" --mtime='2015-10-21 00:00Z' --sort=name --transform 's/^./bin/' .

FROM scratch AS release
COPY --link --from=releaser /out/ /

FROM alpinebase AS devkit-export
...
FROM binaries AS devkit-darwin

FROM binaries AS devkit-freebsd
ENTRYPOINT ["/devkitd"]

FROM binaries AS devkit-windows
...
# The entrypoint script is needed for enabling nested cgroup v2 (https://github.com/khulnasoft/devkit/issues/3265#issuecomment-1309631736)
RUN curl -Ls https://raw.githubusercontent.com/khulnasoft-lab/docker/v20.10.21/hack/dind > /docker-entrypoint.sh \
...
COPY --link hack/fixtures/cni.json /etc/devkit/cni.json
COPY --link hack/fixtures/dns-cni.conflist /etc/devkit/dns-cni.conflist
COPY --link --from=binaries / /usr/bin/
...
RUN adduser -D -u 1000 user \
  && mkdir -p /run/user/1000 /home/user/.local/tmp /home/user/.local/share/devkit \
  && chown -R user /run/user/1000 /home/user \
