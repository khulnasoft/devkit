...
`)
	...
	for _, mode := range []string{"", "min", "max"} {
		t.Run(mode, func(t *testing.T) {
			var target string
			if target == "" {
				target = registry + "/devkit/testwithprovenance:none"
			} else {
				target = registry + "/devkit/testwithprovenance:" + mode
			}
			...
			require.Equal(t, "https://khulnasoftproject.org/devkit@v1", pred.BuildType)
			...
			if isClient {
				...
				if mode == "max" || mode == "" {
					...
					require.Contains(t, args["source"], "devkit_test/")
				} else {
					require.False(t, pred.Metadata.Completeness.Parameters)
			...
			if isGateway {
				...
				require.Contains(t, pred.Materials[0].URI, "docker/devkit_test")
...
`)
	...
	target := registry + "/devkit/testwithprovenance:git"
	...
	if isGateway {
		...
		require.Contains(t, pred.Materials[0].URI, "pkg:docker/devkit_test/")
...
`)
	...
	target := registry + "/devkit/testmultiprovenance:latest"
	...
	for _, p := range []string{"linux/amd64", "linux/arm64"} {
		...
		if isGateway {
			require.Equal(t, 2, len(pred.Materials), "%+v", pred.Materials)
			require.Contains(t, pred.Materials[0].URI, "devkit_test")
...
func testClientFrontendProvenance(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/clientprovenance:latest"
...
func testClientLLBProvenance(t *testing.T, sb integration.Sandbox) {
	...
	target := registry + "/devkit/clientprovenance:llb"
...
`)
	...
	target := registry + "/devkit/testsecretprovenance:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: dir,
			dockerui.DefaultLocalNameContext:    dir,
...
func testOCILayoutProvenance(t *testing.T, sb integration.Sandbox) {
	...
	require.NoError(t, err)
	target := registry + "/devkit/clientprovenance:ocilayout"
...
// https://github.com/khulnasoft/devkit/issues/3562
...
// https://github.com/khulnasoft/devkit/issues/3928
...
`)
	...
	target := registry + "/devkit/testwithprovenance:dup"
