...
import (
	...
	"github.com/khulnasoft/devkit/frontend/dockerfile/parser"
...
func dispatchCopy(d *dispatchState, cfg copyConfig) error {
	...
	for _, src := range cfg.params.SourcePaths {
		...
		if gitRefErr == nil && !gitRef.IndistinguishableFromLocal {
			...
			// would result in undefined behavior: https://github.com/khulnasoft/devkit/pull/387#discussion_r189494717
			u, err := url.Parse(src)
			f := "__unnamed__"
			if err == nil {
