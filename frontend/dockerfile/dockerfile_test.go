...
import (
	...
	v1 "github.com/khulnasoft/devkit/cache/remotecache/v1"
	...
	"github.com/khulnasoft/devkit/frontend/dockerfile/builder"
	...
	"github.com/khulnasoft/devkit/session/upload/uploadprovider"
...
func init() {
	...
	if os.Getenv("FRONTEND_BUILTIN_ONLY") == "1" {
		...
		name := "devkit_test/" + identity.NewID() + ":latest"
...
`, server.URL+"/t.tar.gz"))
	...
	// https://github.com/khulnasoft/devkit/issues/386
	dockerfile = []byte(fmt.Sprintf(`
FROM scratch
...
`)
	...
	target := registry + "/devkit/testonbuild:base"
	...
	target2 := registry + "/devkit/testonbuild:child"
...
`)
	...
	target := registry + "/devkit/testexportdf:multi"
...
`)
	...
	target := registry + "/devkit/testexportdf:latest"
...
`)
	...
	require.NoError(t, err)
	target := registry + "/devkit/testnamedimagecontext:latest"
...
func testNamedImageContextPlatform(t *testing.T, sb integration.Sandbox) {
	...
	// Build a base image and force devkit to generate a manifest list.
	dockerfile := []byte(`FROM --platform=$BUILDPLATFORM alpine:latest`)
	target := registry + "/devkit/testnamedimagecontextplatform:latest"
...
`)
	...
	target := registry + "/devkit/testnamedimagecontexttimestamps:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: dir,
			dockerui.DefaultLocalNameContext:    dir,
	...
	targetDerived := registry + "/devkit/testnamedimagecontexttimestampsderived:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		FrontendAttrs: map[string]string{
...
`)
	...
	target := registry + "/devkit/testsbomscannertarget:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: dir,
			dockerui.DefaultLocalNameContext:    dir,
...
`)
	...
	scannerTarget := registry + "/devkit/testsbomscannerargs:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: scannerDir,
			dockerui.DefaultLocalNameContext:    scannerDir,
...
`)
	...
	target := registry + "/devkit/testsbomscannerargstarget1:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: dir,
			dockerui.DefaultLocalNameContext:    dir,
...
`)
	...
	target = registry + "/devkit/testsbomscannertarget2:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: dir,
			dockerui.DefaultLocalNameContext:    dir,
	...
	target = registry + "/devkit/testsbomscannertarget3:latest"
	_, err = f.Solve(sb.Context(), c, client.SolveOpt{
		LocalDirs: map[string]string{
			dockerui.DefaultLocalNameDockerfile: dir,
			dockerui.DefaultLocalNameContext:    dir,
...
func testReproSourceDateEpoch(t *testing.T, sb integration.Sandbox) {
	...
	if sb.Snapshotter() == "native" {
		t.Skip("the digest is not reproducible with the \"native\" snapshotter because hardlinks are processed in a different way: https://github.com/khulnasoft/devkit/pull/3456#discussion_r1062650263")
	}
	...
	dockerfile := []byte(`# The base image cannot be busybox, due to https://github.com/khulnasoft/devkit/issues/3455
...
`)
	...
	target := registry + "/devkit/testreprosourcedateepoch:" + fmt.Sprintf("%d", tm.Unix())
	...
	for _, l := range manifest.Layers {
		require.Equal(t, fmt.Sprintf("%d", tm.Unix()), l.Annotations["devkit/rewritten-timestamp"])
	}
	// Cache layers must *not* have rewritten-timestamp
	for _, l := range cacheManifest.Layers {
		require.Empty(t, l.Annotations["devkit/rewritten-timestamp"])
	}
...
func testNilContextInSolveGateway(t *testing.T, sb integration.Sandbox) {
	...
	// should not cause devkitd to panic
