...
import (
	...
	"github.com/khulnasoft/devkit/exporter/attestation"
	...
	attestationTypes "github.com/khulnasoft/devkit/util/attestation"
	...
	"github.com/khulnasoft/devkit/util/purl"
	...
	if len(remote.Descriptors) > historyLayers {
		...
		for _, md := range refMeta[historyLayers:] {
			history = append(history, ocispecs.History{
				...
				Comment:   "devkit.exporter.image.v0",
			})
		}
	}
...
func RemoveInternalLayerAnnotations(in map[string]string, oci bool) map[string]string {
	...
	for k, v := range in {
		...
		switch k {
		case labels.LabelUncompressed, "devkit/createdat":
			continue
		default:
...
func getRefMetadata(ref cache.ImmutableRef, limit int) []refMetadata {
	...
	for i, layer := range layerChain {
		...
		if description := layer.GetDescription(); description != "" {
			...
			meta.description = "created by devkit" // shouldn't be shown but don't fail build
		}
