...
import (
	...
	"github.com/khulnasoft/devkit/executor/runcexecutor"
	containerdsnapshot "github.com/khulnasoft/devkit/snapshot/containerd"
	"github.com/khulnasoft/devkit/util/leaseutil"
	"github.com/khulnasoft/devkit/util/network/netproviders"
	"github.com/khulnasoft/devkit/util/winlayers"
	"github.com/khulnasoft/devkit/worker/base"
	wlabel "github.com/khulnasoft/devkit/worker/label"
...
func NewWorkerOpt(root string, snFactory SnapshotterFactory, rootless bool, processMode oci.ProcessMode, labels map[string]string, idmap *idtools.IdentityMapping, nopt netproviders.Opt, dns *oci.DNSConfig, binary, apparmorProfile string, selinux bool, parallelismSem *semaphore.Weighted, traceSocket, defaultCgroupParent string) (base.WorkerOpt, error) {
	...
	c := containerdsnapshot.NewContentStore(mdb.ContentStore(), "devkit")
	...
	lm := leaseutil.WithNamespace(ctdmetadata.NewLeaseManager(mdb), "devkit")
	snap := containerdsnapshot.NewSnapshotter(snFactory.Name, mdb.Snapshotter(snFactory.Name), "devkit", idmap)
