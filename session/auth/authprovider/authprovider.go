...
import (
	...
	"github.com/khulnasoft/devkit/session/auth"
	"github.com/khulnasoft/devkit/util/progress/progresswriter"
...
func (ap *authProvider) FetchToken(ctx context.Context, req *auth.FetchTokenRequest) (rr *auth.FetchTokenResponse, err error) {
	...
	if creds.Secret != "" {
		...
		resp, err := authutil.FetchTokenWithOAuth(ctx, httpClient, nil, "devkit-client", to)
		if err != nil {
