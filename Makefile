...
.PHONY: images
images:
# khulnasoft/devkit:local and khulnasoft/devkit:local-rootless are created on Docker
	hack/images local khulnasoft/devkit
	TARGET=rootless hack/images local khulnasoft/devkit
...
.PHONY: vendor
vendor:
	$(eval $@_TMP_OUT := $(shell mktemp -d -t devkit-output.XXXXXXXXXX))
