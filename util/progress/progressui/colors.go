...
func setUserDefinedTermColors(colorsEnv string) {
	...
	for _, field := range fields {
		...
		if !ok || strings.Contains(v, "=") {
			...
			bklog.L.WithError(err).Warnf("Could not parse DEVKIT_COLORS component: %s", field)
			continue
		}
		k = strings.ToLower(k)
		if c, ok := termColorMap[strings.ToLower(v)]; ok {
			...
			bklog.L.WithError(err).Warnf("Unknown color value found in DEVKIT_COLORS: %s=%s", k, v)
		}
	}
}
...
func readDevkitColorsEnv(colorsEnv string) []string {
	...
	if err != nil {
		bklog.L.WithError(err).Warnf("Could not parse DEVKIT_COLORS. Falling back to defaults.")
		return nil
	}
	return fields
}
...
func parseKeys(k string, c aec.ANSI) {
	...
	case "warning":
		colorWarning = c
	default:
		bklog.L.Warnf("Unknown key found in DEVKIT_COLORS (expected: run, cancel, error, or warning): %s", k)
	}
}
...
func isValidRGB(s []string) bool {
	for _, n := range s {
		...
		if err != nil {
			bklog.L.Warnf("A field in DEVKIT_COLORS appears to contain an RGB value that is not an integer: %s", strings.Join(s, ","))
			return false
		}
		...
		if ok {
			...
			bklog.L.Warnf("A field in DEVKIT_COLORS appears to contain an RGB value that is not within the valid range of 0-255: %s", strings.Join(s, ","))
			return false
		}
	}
	return true
