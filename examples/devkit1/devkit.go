...
func devkit(opt buildOpt) llb.State {
	src := goBuildBase().With(goFromGit("github.com/khulnasoft/devkit", "master"))

	devkitd := src.
		Run(llb.Shlex("go build -o /bin/devkitd ./cmd/devkitd")).Root()
	...
	r := llb.Image("docker.io/library/alpine:latest").With(
		...
		copyFrom(devkitd, "/bin/devkitd", "/bin/"),
