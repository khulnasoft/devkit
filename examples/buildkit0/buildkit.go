...
func devkit(opt buildOpt) llb.State {
	src := goBuildBase().
		Run(llb.Shlex("git clone https://github.com/khulnasoft/devkit.git /go/src/github.com/khulnasoft/devkit")).
		Dir("/go/src/github.com/khulnasoft/devkit")

	devkitd := src.
		Run(llb.Shlex("go build -o /bin/devkitd ./cmd/devkitd"))
	...
	r = copy(devkitd.Root(), "/bin/devkitd", r, "/bin/")
