...
func (sr *immutableRef) ociDesc(ctx context.Context, dhs DescHandlers, preferNonDist bool) (ocispecs.Descriptor, error) {
	...
	if !createdAt.IsZero() {
		...
		desc.Annotations["devkit/createdat"] = string(createdAt)
	}
...
const (
	...
	blobAnnotationsLabelPrefix = "devkit.io/blob/annotation."
	blobMediaTypeLabel         = "devkit.io/blob/mediatype"
)
...
func (sm *sharableMountable) Mount() (_ []mount.Mount, _ func() error, retErr error) {
	...
	if sm.curMounts == nil {
		...
		dir, err := os.MkdirTemp(sm.mountPoolRoot, "devkit")
		if err != nil {
			return nil, nil, err
		}
		defer func() {
