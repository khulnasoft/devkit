...
- [Quick start](#quick-start)
  - [Starting the `devkitd` daemon](#starting-the-devkitd-daemon)
...
### Starting the `devkitd` daemon

You need to run `devkitd` as the root user on the host.

```bash
$ sudo devkitd
```

To run `devkitd` as a non-root user, see [`docs/rootless.md`](docs/rootless.md).

The devkitd daemon supports two worker backends: OCI (runc) and containerd.
...
To start the devkitd daemon using systemd socket activation, you can install the devkit systemd unit files.
...
-   [Buildpacks](https://github.com/tonistiigi/devkit-pack)
...
-   [Nix](https://github.com/reproducible-containers/devkit-nix)
...
External versions of the Dockerfile frontend are pushed to https://hub.docker.com/r/docker/dockerfile-upstream and https://hub.docker.com/r/docker/dockerfile and can be used with the gateway frontend. The source for the external frontend is currently located in `./frontend/dockerfile/cmd/dockerfile-frontend` but will move out of this repository in the future ([#163](https://github.com/khulnasoft/devkit/issues/163)). For automatic build from master branch of this repository `docker/dockerfile-upstream:master` or `docker/dockerfile-upstream:master-labs` image can be used.
...
ctr --namespace=devkit images ls
```

To change the containerd namespace, you need to change `worker.containerd.namespace` in [`/etc/devkit/devkitd.toml`](./docs/devkitd.toml.md).

## Cache

To show local build cache (`/var/lib/devkit`):
...
See [`./docs/devkitd.toml.md`](./docs/devkitd.toml.md).
...
* `mode=<min|max>`: specify cache layers to export (default: `min`)
  * `min`: only export layers for the resulting image
  * `max`: export all the layers of all intermediate steps
* `scope=<scope>`: which scope cache object belongs to (default `devkit`)
* `ignore-error=<false|true>`: specify if error is ignored in case cache export fails (default: `false`)
...
* Any system using environment variables / config files supported by the [AWS Go SDK](https://docs.aws.amazon.com/sdk-for-go/v1/developer-guide/configuring-sdk.html). The configuration must be available for the devkit daemon, not for the client.
...
* `name=<manifest>`: specify name of the manifest to use (default `devkit`)
...
* `name=<manifest>`: name of the manifest to use (default `devkit`)
...
* Any system using environment variables supported by the [Azure SDK for Go](https://docs.microsoft.com/en-us/azure/developer/go/azure-sdk-authentication). The configuration must be available for the devkit daemon, not for the client.
...
`buildctl build` can be called against randomly load balanced the `devkitd` daemon.
...
-   `khulnasoft/devkit:latest`: built from the latest regular [release](https://github.com/khulnasoft/devkit/releases)
-   `khulnasoft/devkit:rootless`: same as `latest` but runs as an unprivileged user, see [`docs/rootless.md`](docs/rootless.md)
-   `khulnasoft/devkit:master`: built from the master branch
-   `khulnasoft/devkit:master-rootless`: same as master but runs as an unprivileged user, see [`docs/rootless.md`](docs/rootless.md)
...
buildctl --addr=podman-container://devkitd build --frontend dockerfile.v0 --local context=. --local dockerfile=. --output type=oci | podman load foo
```
...
buildctl --addr=nerdctl-container://devkitd build --frontend dockerfile.v0 --local context=. --local dockerfile=. --output type=oci | nerdctl load
```
...
# restart devkitd and buildctl so they know JAEGER_TRACE
