...
import (
	...
	traceexec "github.com/khulnasoft/devkit/util/tracing/exec"
...
const (
	...
	// Note that NoProcessSandbox allows build containers to kill (and potentially ptrace) an arbitrary process in the DevKit host namespace.
	// NoProcessSandbox should be enabled only when the DevKit is running in a container as an unprivileged user.
	NoProcessSandbox
)
...
func GenerateSpec(ctx context.Context, meta executor.Meta, mounts []executor.Mount, id, resolvConf, hostsFile string, namespace network.Namespace, cgroupParent string, processMode ProcessMode, idmap *idtools.IdentityMapping, apparmorProfile string, selinuxB bool, tracingSocket string, opts ...oci.SpecOpts) (*specs.Spec, func(), error) {
	...
	if cgroupParent != "" {
		...
		if strings.Contains(cgroupParent, ".slice") && lastSeparator == ":" {
			...
			cgroupsPath = filepath.Join("/", cgroupParent, "devkit", id)
		}
