...
insecure=true
ca=["myca.pem"]
tlsconfigdir=["/etc/devkitd/myregistry"]
...
`
	...
	require.Equal(t, "devkit.sock", cfg.GRPC.Address[0])
	...
	require.Equal(t, cfg.Registries["docker.io"].TLSConfigDir, []string{"/etc/devkitd/myregistry"})
