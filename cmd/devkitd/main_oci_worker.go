...
import (
	...
	"github.com/khulnasoft/devkit/worker/runc"
	...
	case "stargz":
		...
		snFactory.New = func(root string) (ctdsnapshot.Snapshotter, error) {
			...
			fs, err := sgzfs.NewFilesystem(filepath.Join(root, "stargz"),
				sgzCfg,
				// Source info based on the devkit's registry config and session
...
func validOCIBinary() bool {
	...
	_, err1 := exec.LookPath("devkit-runc")
	if err != nil && err1 != nil {
...
// snapshot. This callback returns configuration that is based on devkitd's registry config
