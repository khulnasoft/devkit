...
import (
	...
	"github.com/khulnasoft/devkit/cache/remotecache/azblob"
	"github.com/khulnasoft/devkit/cache/remotecache/gha"
	inlineremotecache "github.com/khulnasoft/devkit/cache/remotecache/inline"
	localremotecache "github.com/khulnasoft/devkit/cache/remotecache/local"
	registryremotecache "github.com/khulnasoft/devkit/cache/remotecache/registry"
	s3remotecache "github.com/khulnasoft/devkit/cache/remotecache/s3"
	"github.com/khulnasoft/devkit/client"
	"github.com/khulnasoft/devkit/cmd/devkitd/config"
	"github.com/khulnasoft/devkit/control"
	"github.com/khulnasoft/devkit/executor/oci"
	"github.com/khulnasoft/devkit/frontend"
	dockerfile "github.com/khulnasoft/devkit/frontend/dockerfile/builder"
	"github.com/khulnasoft/devkit/frontend/gateway"
	"github.com/khulnasoft/devkit/frontend/gateway/forwarder"
	"github.com/khulnasoft/devkit/session"
	"github.com/khulnasoft/devkit/solver"
	"github.com/khulnasoft/devkit/solver/bboltcachestorage"
	...
	"github.com/khulnasoft/devkit/util/archutil"
...
func init() {
	...
	// enable in memory recording for devkitd traces
...
func main() {
	...
	app.Action = func(c *cli.Context) error {
		...
		lockPath := filepath.Join(root, "devkitd.lock")
	...
	if err := app.Run(os.Args); err != nil {
		fmt.Fprintf(os.Stderr, "devkitd: %+v\n", err)
		os.Exit(1)
	}
}
...
func defaultConfigPath() string {
	if isRootlessConfig() {
		return filepath.Join(appdefaults.UserConfigDir(), "devkitd.toml")
	}
	return filepath.Join(appdefaults.ConfigDir, "devkitd.toml")
}
...
func isRootlessConfig() bool {
	isRootlessConfigOnce.Do(func() {
		...
		// if devkitd is being executed as the mapped-root (not only EUID==0 but also $USER==root)
...
func newWorkerController(c *cli.Context, wiOpt workerInitializerOpt) (*worker.Controller, error) {
	...
	if nWorkers == 0 {
		return nil, errors.New("no worker found, rebuild the devkit daemon?")
	}
